...
void CScene::BuildDefaultLightsAndMaterials()
{
	...
	m_pLights[0].m_xmf3Position = XMFLOAT3(30.0f, 80.0f, 30.0f);
	...
	m_pLights[1].m_xmf3Position = XMFLOAT3(-50.0f, 80.0f, -5.0f);
	...
	m_pLights[3].m_xmf3Position = XMFLOAT3(50.0f, 80.0f, 30.0f);
...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	CLineObjectsShader* pLineObjectShader = new CLineObjectsShader();
	pLineObjectShader->CreateShader(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature);
	pLineObjectShader->BuildObjects(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, m_pTerrain);
	m_ppShaders[1] = pLineObjectShader;
...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[8].NumDescriptors = 2;
	pd3dDescriptorRanges[8].BaseShaderRegister = 14; //t14 gtxtTerrainBaseTexture, t15 gtxtTerrainDetailTexture
	...
	pd3dDescriptorRanges[10].BaseShaderRegister = 16; //t16 gtxtWater
	pd3dDescriptorRanges[10].RegisterSpace = 0;
	pd3dDescriptorRanges[10].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[11].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[11].NumDescriptors = 1;
	pd3dDescriptorRanges[11].BaseShaderRegister = 17; //t16 gtxtCubemap
...
void CScene::AnimateObjects(float fTimeElapsed)
{
	...
	if (m_ppShaders[1]->m_ppObjects[0]->fly == true) {
		m_ppShaders[2]->m_ppObjects[0]->fly = true;
	...
	if (m_ppShaders[2]->m_ppObjects[0]->attack && m_ppShaders[2]->m_ppObjects[1]->ready == true && m_ppShaders[2]->m_ppObjects[0]->fly == false) {
		m_ppShaders[2]->m_ppObjects[2]->ready = false;
		m_ppShaders[2]->m_ppObjects[2]->SetPosition(m_ppShaders[2]->m_ppObjects[0]->GetPosition());
		m_ppShaders[2]->m_ppObjects[2]->m_xmDirect = Vector3::Normalize(Vector3::Subtract(m_pPlayer->GetPosition(), m_ppShaders[2]->m_ppObjects[2]->GetPosition()));
...
void CScene::OnPrepareRender(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	if (m_pd3dcbMaterials)
	{
		...
		pd3dCommandList->SetGraphicsRootConstantBufferView(1, d3dcbMaterialsGpuVirtualAddress); //Materials
	}
}
...
void CScene::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (pCamera->GetMode() == THIRD_PERSON_CAMERA) {
		...
		pCamera->SetOffset(XMFLOAT3(-2.0f, 2.4f, -0.1f));
...
void CScene::BoundingCheck() {
	...
	for (int i = 0; i < m_ppShaders[2]->m_nObjects; i++) {
		m_ppShaders[2]->m_ppObjects[i]->m_AABB.Center = m_ppShaders[2]->m_ppObjects[0]->GetPosition();
	}
	if (m_ppShaders[2]->m_ppObjects[0]->m_AABB.Intersects(m_ppShaders[2]->m_ppObjects[1]->m_AABB) && m_ppShaders[2]->m_ppObjects[1]->ready == false)
	{
		XMFLOAT3 ene = m_ppShaders[2]->m_ppObjects[0]->GetPosition();
		XMFLOAT3 ply = m_pPlayer->GetPosition();
		XMFLOAT3 ToTarget = Vector3::Subtract(ene, ply);

		if (Vector3::Length(ToTarget) < 30) {
			m_ppShaders[2]->m_ppObjects[0]->fly = true;
			m_ppShaders[2]->m_ppObjects[1]->ready = true;
			m_pPlayer->point++;
		}
		else {
			m_ppShaders[2]->m_ppObjects[1]->ready = true;
