...
void CShader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, int nPipelineState)
{
	...
	d3dPipelineStateDesc.StreamOutput = CreateStreamOuputState();
...
CCubeMappingShader::CCubeMappingShader(int nMapSize)
{
	if (nMapSize)
		m_nCubeMapSize = nMapSize;
	else
		m_nCubeMapSize = 256;
}

CCubeMappingShader::~CCubeMappingShader()
{
}

D3D12_INPUT_LAYOUT_DESC CCubeMappingShader::CreateInputLayout()
{
...
D3D12_SHADER_BYTECODE CCubeMappingShader::CreateVertexShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSLightingColor", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CCubeMappingShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSCubeMapping", "ps_5_1", &m_pd3dPixelShaderBlob));

}
void CCubeMappingShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		m_ppObjects[i]->SetPosition(439.829132, 115.111160, 396.016663);
...
void CCubeMappingShader::ReleaseObjects()
{
...
void CCubeMappingShader::ReleaseUploadBuffers()
{
	CObjectsShader::ReleaseUploadBuffers();
}

void CCubeMappingShader::OnPreRender(ID3D12Device* pd3dDevice, ID3D12CommandQueue* pd3dCommandQueue, ID3D12Fence* pd3dFence, HANDLE hFenceEvent, CScene* pScene)
{
	for (int i = 0; i < m_nObjects; i++)
	{
...
void CCubeMappingShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
...
D3D12_DEPTH_STENCIL_DESC CVisionShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	d3dDepthStencilDesc.DepthEnable = FALSE;
	d3dDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
...
void CVisionShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	m_nObjects = 3;
	m_ppObjects = new CGameObject * [m_nObjects];
	...
	m_ppObjects[0]->SetChild(pGunshipModel);
	m_ppObjects[0]->SetPosition(XMFLOAT3({ 409.692932,115.111160,107.538094 }));
	m_ppObjects[0]->SetScale(1.7f, 1.7f, 1.7f);
	m_ppObjects[0]->PrepareAnimate();
	///
