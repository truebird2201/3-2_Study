...
void CPlayer::Rotate(float x, float y, float z)
{
	...
	if ((nCurrentCameraMode == FIRST_PERSON_CAMERA) || (nCurrentCameraMode == THIRD_PERSON_CAMERA))
	{
		...
		if (z != 0.0f)
		{
			XMMATRIX xmmtxRotate = XMMatrixRotationAxis(XMLoadFloat3(&m_xmf3Right), XMConvertToRadians(z));
			m_xmf3Look = Vector3::TransformNormal(m_xmf3Look, xmmtxRotate);
...
void CAirplanePlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
	...
	if (xmf3PlayerPosition.y < fHeight)
	{
		...
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, XMFLOAT3{ m_xmf3Velocity.x / 1000,m_xmf3Velocity.y / 1000,m_xmf3Velocity.z / 1000 });
	}
