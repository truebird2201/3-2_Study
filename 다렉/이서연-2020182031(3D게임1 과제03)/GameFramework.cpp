...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_CONTROL:					// 총알 쏘기
			if ((m_pPlayer->nowB>0)) {
				m_pScene->m_ppBullets[m_pPlayer->nowB-1]->SetPosition(m_pPlayer->GetPosition());
				m_pScene->m_ppBullets[m_pPlayer->nowB-1]->ready = false;
				m_pScene->m_ppBullets[m_pPlayer->nowB - 1]->m_xmDirect = m_pPlayer->GetLookVector();
				m_pScene->m_ppBullets[m_pPlayer->nowB-1]->m_xmDirect.y += 0.5;
				m_pPlayer->nowB--;
			}
			break;
		case VK_SPACE:
			m_pScene->m_ppBullets[0]->Move(m_pScene->m_ppBullets[0]->m_xmDirect, true);
			break;
		default:
			break;
		}
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f) && m_pPlayer->crash == false)
		{
			if (cxDelta || cyDelta)
			{
...
{    
	...
	if (m_pScene->m_ppGameObjects[0]->goal) {
		HFONT font;
