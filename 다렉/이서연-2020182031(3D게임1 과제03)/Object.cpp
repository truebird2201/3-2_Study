...
CShader* CMaterial::m_pTerrainShader = NULL;
...
void CMaterial::PrepareShaders(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	...
	m_pTerrainShader->CreateShader(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
...
void CGameObject::SetAABBMesh(CMesh* pAABBMesh)
{
	if (m_pAABBMesh) m_pAABBMesh->Release();
	m_pAABBMesh = pAABBMesh;
	if (m_pAABBMesh) m_pAABBMesh->AddRef();
}
...
void CGameObject::OnPlayerUpdateCallback(float fTimeElapsed)
{
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)m_pPlayerUpdatedContext;

	float fHeight = pTerrain->GetHeight(GetPosition().x, GetPosition().z) +
		6.0f;
	...
	if (m_nMaterials > 0)
	{
		for (int i = 0; i < m_nMaterials; i++)
		{
			...
			for (int j = 0; j < m_nMeshes; ++j) {
				if (m_ppMeshes[j])
				{
					m_ppMeshes[j]->Render(pd3dCommandList, j);
				}
			}
		}
	}
	if (m_pAABBMesh && check == true) {
		auto mtx_aabb = XMMatrixScaling(m_AABBExtents.x, m_AABBExtents.y, m_AABBExtents.z);
		auto mtx = XMMatrixMultiply(mtx_aabb, XMLoadFloat4x4(&m_xmf4x4World));
		XMFLOAT4X4 xmf4x4World;
		XMStoreFloat4x4(&xmf4x4World, XMMatrixTranspose(mtx));
		pd3dCommandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4World, 0);

		m_pAABBMesh->Render(pd3dCommandList, 0);
	}
	...
	xmf4Color) : CGameObject(0)
{
	...
	pMaterialColors->m_xmf4Diffuse = XMFLOAT4{ 0.2f, 0.3f, 0.4f, 1.0f };
...
void CHellicopterObject::FollowPlayer(float fTimeElapsed) {
	if (fly == false) {
...
void CBulletObject::Move(const XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	...
	else
	{
		...
		m_AABBCenter = Vector3::Add(m_AABBCenter, xmf3Shift);
	}
